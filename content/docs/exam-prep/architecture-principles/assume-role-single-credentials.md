...
1. 나는 나만의 개인 사원증(내 고유 자격 증명)을 들고 다닙니다 — 다른 사람과 공유하지 않습니다.
2. 운영 계정의 리소스가 필요할 때, AWS 시스템(STS)에 내 사원증을 찍습니다.
3. AWS는 "당신은 운영 계정에 들어갈 권한이 있는 사람이군요"라며 나만을 위한 임시 출입 카드를 1시간 동안만 빌려줍니다.
4. 1시간 뒤면 그 카드는 자동으로 사라집니다.
...
{{< callout type="info" >}}
이 방식은 "키를 나눠 쓰는 것"이 아니라 **"나의 인증 체계를 여러 곳에서 통용되게 만드는 것"** 이라고 이해하면 됩니다. 자격 증명(키)은 절대 이동하거나 복제되지 않고, 그 자격 증명을 근거로 매번 새로운 임시 권한이 발급될 뿐입니다.
{{< /callout >}}
...
1. **인증(누구인가?)**: 개발자 개인별로 단 하나의 고유한 키(IAM 사용자)를 가집니다 — 인증은 중앙(개발 계정)에 집중됩니다.
2. **권한(무엇을 할 수 있는가?)**: 각 시스템(운영 계정, 개발 계정 등)은 자신만의 역할(Role)을 미리 정의해둡니다.
3. **결정(접근 가능한가?)**: 개발자가 시스템에 접근을 시도하면, 시스템은 그 개발자의 정보를 바탕으로 "이 역할을 맡을(Assume) 권한이 있는 사람인지" 신뢰 정책으로 확인해 부여하거나 거절합니다.
...
{{< callout type="info" >}}
이 구조가 바로 AWS가 강조하는 **최소 권한의 원칙**과 **관리 오버헤드 최소화**를 동시에 만족시키는 아키텍처의 핵심입니다.
{{< /callout >}}
